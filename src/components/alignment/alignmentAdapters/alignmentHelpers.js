...
export const getTokensFromProskomma = async ({bibleDocument}) => {
  const SCOPE_SPANWITHATTS_PREFIX_FORMAT = "spanWithAtts[.]*";
  ...
  let alignments = bibleDocument && bibleDocument.cv[0].tokens.map((token) => {
    ...
    if (isWord == false) {
      if (! token.scopes.find(scope => scope.match(SCOPE_SPANWITHATTS_PREFIX_FORMAT))) {
        // Unaligned text?
        // There are NO ATTRIBUTES, but we accept this token a a word
        // (until tokenized/filtered below!  see below!)
        isWord = true;
      }
    }
